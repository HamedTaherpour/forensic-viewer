...
const renderHotspotShape = (
  ...
  shapeElement.addEventListener("mouseenter", () => {
    ...
    shapeElement.style.transform = shapeElement.style.transform.replace(
      "scale(1)",
      "scale(1.1)"
    );
...
const PanoramaViewerAdvanced: React.FC<PanoramaViewerAdvancedProps> = ({
  ...
  const pannellumInstanceRef = useRef<ReturnType<
    typeof window.pannellum.viewer
  > | null>(null);
  ...
  const updateHotspotSizes = (hfov: number) => {
    ...
    const hotspotMarkers =
      viewerRef.current.querySelectorAll(".hotspot-marker");
  ...
  useEffect(() => {
    ...
    try {
      ...
      const pannellumHotspots = hotspots.map((hotspot) => ({
        ...
        createTooltipFunc: (hotSpotDiv: HTMLElement) => {
          ...
          marker.className = `hotspot-marker ${
            editMode ? "hotspot-marker-edit" : ""
          } ${isSelected ? "hotspot-marker-selected" : ""}`;
          ...
          if (hotspot.shape) {
            ...
            console.warn(
              "Hotspot",
              hotspot.id,
              "is missing shape configuration"
            );
          }
          ...
          if (!editMode) {
            ...
            editText.textContent = `${hotspot.text} - Click to edit`;
      ...
      pannellumInstanceRef.current = window.pannellum.viewer(
        viewerRef.current,
        viewerConfig
      );
      ...
      {isLoading && (
        <div className="absolute inset-0 flex items-center justify-center bg-black/10 z-10 rounded-lg">
          <div className="flex flex-col items-center gap-3">
            ...
            <p className="text-sm text-foreground/70">Loading panorama...</p>
          </div>
        </div>
      )}
      ...
      {error && (
        <div className="absolute inset-0 flex items-center justify-center bg-red-500/10 z-10 rounded-lg">
          <div className="text-center p-6 bg-background rounded-lg shadow-lg">
            ...
            <p className="text-red-600 font-semibold mb-1">Load error</p>
